...
class StarCraft2Env(MultiAgentEnv):
    ...
    def get_obs_agent(self, agent_id):
        ...
        # reshape the observation features in the case of entity mode
        if self.obs_entity_mode:
            entity_feats_dim = max(enemy_feats_dim[1], ally_feats_dim[1]) # uniform the number of feats
            enemy_feats_dim = (enemy_feats_dim[0], entity_feats_dim+2)    # +2 in [1] for adding IS_AGENT AND IS_SELF feats
            ally_feats_dim  = (ally_feats_dim[0]+1,  entity_feats_dim+2)  # +2 in [0] for considering SELF in the allies
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                if (
                    ...
                    if self.obs_all_health:
                        ...
                        if self.shield_bits_enemy > 0:
                            ...
                            ind += 1
                        elif self.obs_entity_mode and self.shield_bits_ally: # pad in case of entity mode
                            ind += 1
                ...
                # IS_AGENT and IS_SELF feature
                if self.obs_entity_mode:
                    enemy_feats[e_id, -2] = 0 # is agent
                    enemy_feats[e_id, -1] = 0 # is self

            # Ally features
            al_ids = [
                al_id for al_id in range(self.n_agents) 
                if self.obs_entity_mode or al_id != agent_id
            ]
            for i, al_id in enumerate(al_ids):
                ...
                if (
                    ...
                    if self.obs_all_health:
                        ...
                        if self.shield_bits_ally > 0:
                            ...
                            ind += 1
                        elif self.obs_entity_mode and self.shield_bits_enemy: # pad in case of entity mode
                            ind += 1
                ...
                # IS_AGENT and IS_SELF feature
                if self.obs_entity_mode:
                    ally_feats[i, -2] = 1 # is agent
                    ally_feats[i, -1] = agent_id == al_id # is self
    ...
    def get_state(self):
        ...
        if self.state_entity_mode:
            nf_al = nf_en = (
                5 + # health, x, y, energy/weapon, is_agent
                max(self.shield_bits_ally, self.shield_bits_enemy) + 
                self.unit_type_bits
            )
        ...
        for al_id, al_unit in self.agents.items():
            if al_unit.health > 0:
                ...
                if (
                    ...
                    ally_state[al_id, 3] = al_unit.energy / max_cd  # energy
                else:
                    ally_state[al_id, 3] = (
                ...
                if self.shield_bits_ally > 0:
                    ...
                    ind += 1
                elif self.state_entity_mode and self.shield_bits_enemy:
                    ind += 1
        ...
        for e_id, e_unit in self.enemies.items():
            if e_unit.health > 0:
                ...
                # skip the 3rd feature (energy/weapon cooldown() in case of ent mode
                ind = 4 if self.state_entity_mode else 3 
                if self.shield_bits_enemy > 0:
                    max_shield = self.unit_max_shield(e_unit)
                    ...
                    ind += 1
                elif self.state_entity_mode and self.shield_bits_ally > 0:
                    ind += 1
                ...
                if self.state_entity_mode:
                    ally_state[al_id, -1] = 0

        state = np.append(ally_state.flatten(), enemy_state.flatten())
        if self.state_last_action and not self.state_entity_mode:
            state = np.append(state, self.last_action.flatten())
        if self.state_timestep_number and not self.state_entity_mode:
    ...
    def get_obs_size(self):
        ...
        if self.obs_entity_mode:
            final_dim = (self.n_enemies + self.n_agents) * (max(n_enemy_feats, n_ally_feats)+2)
        else:
            final_dim = move_feats + enemy_feats + ally_feats + own_feats
    ...
    def get_state_size(self):
        ...
        if self.state_entity_mode:
            nf =  5 + max(self.shield_bits_ally, self.shield_bits_enemy) + self.unit_type_bits
            return (self.n_enemies+self.n_agents)*nf
    ...
    def get_env_info(self):
        env_info = {
            ...
            "n_entities": self.n_agents + self.n_enemies,
            "episode_limit": self.episode_limit
        }
        if self.obs_entity_mode:
            env_info["obs_entity_feats"] = int(
                self.get_obs_size()/(self.n_agents+self.n_enemies)
            )
        if self.state_entity_mode:
            env_info["state_entity_feats"] = int(
                self.get_state_size()/(self.n_agents+self.n_enemies)
            )
        return env_info
